# Integrates the brain (motor cortex) sketch data across samples using CCA,
...
data_in_dir <- "data/08_sketch_pca/brain/"

plots_dir <- "plots/10_integration/"
dir.create(plots_dir, showWarnings = F,
           recursive = T)

data_out_dir <- "data/10_integration/brain/"
...
p <- DimPlot_scCustom(obj,
                      ...
                      colors_use = JCO_Four())
ggsave(p,
       filename = paste0(plots_dir, "brain_cca_umap_dimplot_tissue.png"),
       units = "in", dpi = 600,
       height = 5, width = 6)
...
p <- DimPlot_scCustom(obj,
                      ...
                      colors_use = DiscretePalette_scCustomize(6,
                                                               palette = "ditto_seq"))
ggsave(p,
       filename = paste0(plots_dir, "brain_cca_umap_dimplot_batch.png"),
       units = "in", dpi = 600,
       height = 5, width = 6)
...
p <- DimPlot_scCustom(obj,
                      ...
                      reduction = "cca_umap")
ggsave(p,
       filename = paste0(plots_dir, "brain_cca_umap_dimplot_id.png"),
       units = "in", dpi = 600,
       height = 5, width = 8)
