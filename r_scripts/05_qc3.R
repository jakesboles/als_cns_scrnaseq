...
meta <- read.csv(paste0(proj_dir, "tab_data/metadata.csv"))

plots_dir <- paste0(proj_dir, "plots/05_qc3/")
...
pre_stats_files <- list.files(paste0(proj_dir, 
                                     "tab_data/02_qc1"),
                              full.names = T)
...
contam <- read.csv(paste0(proj_dir, 
                          "tab_data/00_soupx/contamination_fraction_fixed.csv"))[1:90, ] %>%
        ...
        if (deparse(substitute(dep_var)) == "Median_log10GenesPerUMI") {
          ...
          if (deparse(substitute(dep_var)) == "retained_percent") {
            ...
            if (deparse(substitute(dep_var)) == "retained_count") {
              ...
              if(deparse(substitute(dep_var)) == "contamination_fraction") {
...
plot(pre_stats, Tissue, contamination_fraction) + 
  plot(pre_stats, Group, contamination_fraction) +
  plot(pre_stats, Batch, contamination_fraction) + 
  plot_layout(design = design)
...
post_stats_files <- list.files(paste0(proj_dir, "tab_data/03_qc2/"),
                               full.names = T)
...
counts <- read.csv(paste0(proj_dir, "tab_data/03_qc2/filtered_counts.csv")) %>%
  dplyr::rename("ID" = "id", "Tissue" = "tissue") %>%
  mutate(Tissue = factor(Tissue,
                         levels = c("Motor cortex", "Cervical spinal cord", "Skeletal muscle"),
                         labels = c("Motor cortex", "Cervical\nspinal cord", "Skeletal muscle")),
         ID = str_replace_all(ID, "GWF-", "GWF"))
