...
files <- list.files("data/19_more_cleanup/",
                    full.names = T)
files <- files[str_detect(files, "brain_|sc_")]
...
for (i in seq_along(objs)){
  ...
  message(paste0("Data set ", i, " out of 2"))
...
p <- ElbowPlot(obj,
               ndims = 50)
ggsave(p,
       filename = paste0(plots_dir, "cns_only_nosketch_pca_elbow.png"),
       units = "in", dpi = 600,
       bg = "white",
       height = 6, width = 6)
...
Iterate_PC_Loading_Plots(obj,
                         file_path = plots_dir,
                         file_name = "cns_only_nosketch_pca_loadings")
...
obj <- IntegrateLayers(obj,
                       ...
                       reference = which(Layers(obj, search = "data") %in%
                                           c("data.GBB-23-11_b.1", "data.GWF19-47_b.1",
...
for (group in c("final_label2", "Batch", "tissue", "orig.ident", "Group")){
  ...
  p <- DimPlot_scCustom(obj,
                        ...
                        group.by = group)
  ggsave(p,
         filename = paste0(plots_dir, file1, "_cns_only_harmonyintegration_nosketch_umap2.png"),
         units = "in", dpi = 600,
         height = 8, width = w)
...
saveRDS(obj,
        file = paste0(data_dir, "cns_only_harmonyintegration_nosketch.rds"))
