...
for (i in c("brain", "sc", "muscle")) {
  ...
  celltypes <- celltypes[file.exists(paste0(results_dir, "/", i, "/", celltypes, "/sALS_vs_Control_lfc_shrunk.csv"))]
  celltypes <- celltypes[file.exists(paste0(results_dir, "/", i, "/", celltypes, "/C9orf72_vs_Control_lfc_shrunk.csv"))]
  
  for (j in seq_along(celltypes)){
    
    message(celltypes[j])
    
    sals <- read.csv(paste0(results_dir, "/", i, "/", celltypes[j], "/sALS_vs_Control_lfc_shrunk.csv"))
    
    c9 <- read.csv(paste0(results_dir, "/", i, "/", celltypes[j], "/C9orf72_vs_Control_lfc_shrunk.csv"))
    ...
    if (sals_res@result$p.adjust %>% min() < 0.05){
      p <- dotplot(sals_res,
                   ...
                   size = "setSize")
      ggsave(p,
             filename = paste0(results_dir, "/", i, "/", celltypes[j], "/sALS_vs_Control_GSEA.png"),
             units = "in", dpi = 600,
             height = 10, width = 8)
    }
    ...
    if (c9_res@result$p.adjust %>% min() < 0.05){
      p <- dotplot(c9_res,
                   ...
                   size = "setSize")
      ggsave(p,
             filename = paste0(results_dir, "/", i, "/", celltypes[j], "/C9orf72_vs_Control_GSEA.png"),
             units = "in", dpi = 600,
             height = 10, width = 8)
    }
    
    write.csv(as.data.frame(sals_res@result),
              file = paste0(results_dir, "/", i, "/", celltypes[j], "/sALS_vs_Control_GSEA.csv"))
    
    write.csv(as.data.frame(c9_res@result),
              file = paste0(results_dir, "/", i, "/", celltypes[j], "/C9orf72_vs_Control_GSEA.csv"))
  }
}
